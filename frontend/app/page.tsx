...
import { cancelOrder, confirmOrderCancel, createOrder, listAccounts, listOrders, listPositions, type Account, type Order, type Position } from "../lib/api";
...
export default function Dashboard() {
  ...
  async function loadAccountData(nextToken: string, selectedAccountId?: string) {
    const [nextAccounts, nextOrders] = await Promise.all([listAccounts(nextToken), listOrders(undefined, nextToken)]);
    setAccounts(nextAccounts);
    setOrders(nextOrders);
    const selected = selectedAccountId && nextAccounts.some((a) => a.id === selectedAccountId) ? selectedAccountId : nextAccounts[0]?.id ?? "";
    setAccountId(selected);
    setPositions(selected ? await listPositions(selected, nextToken) : []);
  }
  ...
  useEffect(() => {
    ...
    loadAccountData(token, accountId).catch((err: Error) => { if (!cancelled) setError(err.message); }).finally(() => { if (!cancelled) setLoading(false); });
  ...
  const openOrders = useMemo(() => new Set(["PENDING", "ACCEPTED", "PARTIALLY_FILLED", "CANCEL_PENDING"]), []);
  ...
  async function handleOrder(event: FormEvent) {
    event.preventDefault(); if (!token || !accountId) return;
    setLoading(true); setError(""); setMessage("");
    try {
      const created = await createOrder({ account_id: accountId, client_order_id: `WEB-${Date.now()}`, symbol: symbol.trim(), side, order_type: orderType, time_in_force: tif, quantity, ...(orderType === "LIMIT" || orderType === "STOP_LIMIT" ? { limit_price: limitPrice } : {}) }, token);
  ...
  async function handleCancel(order: Order) {
    ...
    try {
      const pending = await cancelOrder(order.id, token);
      setOrders((current) => current.map((item) => item.id === order.id ? { ...item, status: pending.status } : item));
      const confirmed = await confirmOrderCancel(order.id, token);
      setOrders((current) => current.map((item) => item.id === order.id ? { ...item, status: confirmed.status } : item));
      setMessage(`Order ${order.client_order_id} cancelled`);
    } catch (err) { setError(err instanceof Error ? err.message : "Order cancellation failed"); }
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <section className="workspace"><div>
        <div className="card"><div className="panel-title"><h2>Equity curve</h2><span className="badge">PAPER</span></div><div className="chart empty-chart">Live historical equity series will appear here as portfolio snapshots are recorded.</div></div>
